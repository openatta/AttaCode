...
一段 = 转录里的一件事：一次用户输入、一段模型回答、一次工具调用（标题 + 它的结果）、
一条通知。段与段之间留一行空，**段内不留**——工具的标题和结果讲的是同一件事，中间劈开
反而更难读。不分段的话它们首尾相接糊成一片，人得靠颜色和前缀去猜边界在哪。

两条不变量，改这块之前先记住：

1. **一条 entry 就是屏幕上的一行。** `render_body` 按 `entries.take(视口高度)` 取，一条
   画一行；而 ratatui 的 `Line` 对里面的 `\n` 不是换行、是**直接吞掉**
   （`Line::from("aaa\nbbb")` 画出来是 `aaabbb`，连空格都不给）。所以多行文本进转录之前
   必须按行拆开——`bridge::reducer::push_lines` 就是那一步。
2. **段与段之间那行空是一条真 entry**（`LineKind::Spacer`），不是渲染时插进去的。整个
   滚动模型（`total_lines`、翻页步长、`scroll.offset`）都建立在第 1 条上，渲染时凭空多画
   一行会让"跳过 N 条"不再等于"跳过 N 行"，翻页越翻越偏。

`TranscriptEntry::starts_segment` 标出每一段的第一行。**别靠相邻推断段边界**：恢复出来的
转录里，两次相邻的用户提交（发一句、Ctrl+C、再发一句）之间什么都没有，按相邻拼会把两次
提交粘成一条——`crates/app` 的输入历史和 sticky header 都栽在过这里。
