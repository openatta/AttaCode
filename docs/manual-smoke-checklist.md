...
学的是 Claude Code 的 `/btw`。需求与行为规格见 `docs/reqs/2026-09-02-btw-side-question.md`。

先正常聊两句（让会话里有点东西），然后：

- [ ] `/btw 我们刚才在聊什么` → **转录区压到约一半**，下半屏出现带边框的侧问区，
      状态区/输入区/底栏/子代理条**全被盖住**
- [ ] 答案是**流式**长出来的；还没开始时显示"想一下……"，底下那行写着"正在回答"
- [ ] 侧问区自己那条状态栏写清楚了能按什么（↑↓ 滚动 · Esc/Enter 退出）
- [ ] **独占**：打字不落进草稿，回车不提交草稿，切模型之类一律无效
- [ ] `Esc` 退出 → 主 UI 完整恢复，**退出前的草稿还在**
- [ ] 侧问的问和答**不出现在转录区**

无工具 / 不进历史（这两条是它和"再问一句"的根本区别）：

- [ ] `/btw 看看 src 下有几个文件` → 明确说自己没有工具、够不着，**不编一个数出来**
- [ ] `/btw 你刚才读的那个文件叫什么` → 答得出来（它看得见已有的工具结果）
- [ ] 侧问几次之后 `/cost`，主上下文用量**不因侧问上涨**

早前问答：

- [ ] 连问三次 → 上方以暗色列出早前的问题
- [ ] `←` 往旧翻、`→` 往新翻；标题变成"第 N 条（往回）"
- [ ] `x` 清空早前列表
- [ ] `/btw` **不带问题** → 重新打开，停在最近一次问答上
- [ ] 退出 AttaCode 再进来 → 早前问答不在了（只活在本次进程）

turn 跑着的时候（这是 `/btw` 存在的理由）：

- [ ] 发一个长任务，**跑到一半**打开 `/btw` 问一句 → 主 turn **不被打断**
- [ ] 退出侧问区之后，转录区里那一轮还在继续长
- [ ] 侧问区开着时 `Ctrl+C` 仍能中断主 turn
- [ ] **看不见 spinner 和子代理进度是预期的** —— CC 也是这样，是这个形态的代价
- [ ] 但**等人答的请求不是进度**：让主 turn 跑到一个要审批的工具（比如写 `.env`），
      在侧问区里等 → 状态栏那一行变成告警色的"外面有 1 个请求在等你答"，
      流式回答期间也照显；退出去之后那个审批框还在、答得了

边界：

- [ ] 刚起手、一句话都没说就 `/btw` → 说"还没有可问的上下文"，而不是留一个空框
- [ ] 问一句长的，**答到一半按 `Esc`** → 那次请求当场断开；`/btw` 空参翻不到它
      （半截答案不留，否则下一问会把一句没说完的话当上下文带上去）
- [ ] 上一问还在流的时候 `Esc` 出来、马上再 `/btw` 问下一问 → 前一问的字**不会**
      落进新那一问的框里，新那一问的"在想"也不会被提前收掉
- [ ] `x` 在侧问区**外面**打是普通输入（打 "box" 不丢字母）
